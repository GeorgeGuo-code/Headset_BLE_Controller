...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    if (i2c_write(st.reg->fifo_en, data, 1))
        return -1;
    vTaskDelay(pdMS_TO_TICKS(test.wait_ms));
    data[0] = 0;
    if (i2c_write(st.reg->fifo_en, data, 1))
        return -1;
...
/* Same FIFO read as mpu_dmp_get_data, but returns the raw normalized
 * quaternion (body->world) instead of Euler angles. Lets callers do full
 * 3-D relative-orientation math and avoid Euler singularities. */
uint8_t mpu_dmp_get_quat(float *w,float *x,float *y,float *z)
{
	float q0=1.0f,q1=0.0f,q2=0.0f,q3=0.0f;
	unsigned long sensor_timestamp;
	short gyro[3], accel[3], sensors;
